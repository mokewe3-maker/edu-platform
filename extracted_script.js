...
};const user = JSON.parse(sessionStorage.getItem('edu_current_user') || 'nucol');
...
// ===== 初始化用户信息 =====
document.getElementById('teacherAvatar').textcontent = (user.name||'师')[0];
document.getElementById('teacherName').textcontent = user.name || '教师';
document.getElementById('teacherNameTop').textcontent = user.name || '老师';
document.getElementById('teacherSubject').textcontent = (user.subject || '') + ' 教师';
...
function loadInEmbed(url, title, sites) {
  ...
  if (sites && sites.length > 0) {
    ...
    if (titleEl) titleEl.textcontent = '🌐 ' + title + ' - 选择平台直接预览';
    contentEl.innerHTML = `
          ...
          <div style="border:1.5px solid #e2e8f0;border-radius:12px;overflow:hidden;transition:all 0.2s;cursor:pointer;background:#fff;"
            ...
            onclick="embedPreviewSite('${site.url.replace(/'/g, "\\'")}', '${site.name.replace(/'/g, "\\'")}')">
              <div style="padding:14px;text-align:center;">
                <div style="font-size:26px;margin-bottom:6px;">${site.in}</div>
...
function renderTextResults(query) {
  const resultsEl = document.getElementById('net-results');
  const sources = [
    { name:'百度百科', in:'📚', color:'#2932e1', desc:'知识点详解、定义、定理、公式', url:`https://baike.baidu.m/item/${endeURIComponent(query)}` },
    { name:'百度文库', in:'📄', color:'#4a7dff', desc:'教材、教案、课件、习题文档', url:`https://wenku.baidu.m/search?word=${endeURIComponent(query)}&lm=1` },
    { name:'知乎', in:'💬', color:'#00b42a', desc:'名师解读、学习方法、经验分享', url:`https://www.zhihu.m/search?type=content&q=${endeURIComponent(query)}` },
    { name:'学科网', in:'🏫', color:'#e53e3e', desc:'中小学教案、试题、课件资源库', url:`https://www.zxxk.m/search/?KeyWord=${endeURIComponent(query)}` },
    { name:'惠教资源平台', in:'🌐', color:'#059669', desc:'优质教学资源、课件、教案下载', url:`https://www.huijiaoyu.m/search?q=${endeURIComponent(query)}` },
    { name:'B站专栏', in:'📝', color:'#ff6b9d', desc:'学习UP主图文教程、笔记分享', url:`https://search.bilibili.m/article?keyword=${endeURIComponent(query)}` },
    { name:'我要查词典', in:'🔤', color:'#6366f1', desc:'字词典释义、近反义词、例句', url:`https://www.5156edu.m/searchindex/?searchWord=${endeURIComponent(query)}` },
    { name:'国家智慧教育', in:'🌍', color:'#2563eb', desc:'教育部官方中小学课程资源', url:`https://www.smartedu.cn/search?query=${endeURIComponent(query)}` },
    { name:'可汗学院(中文)', in:'🌏', color:'#14b8a6', desc:'国际精品课程、练习题库', url:`https://zh.khanacademy.org/search?search_again=1&page=1&q=${endeURIComponent(query)}` },
    { name:'学习强国', in:'⭐', color:'#dc2626', desc:'权威学习资源、名师课堂', url:`https://www.xuexi.cn/search.html?searchText=${endeURIComponent(query)}` }
  ];
  // 保存到全局变量
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;">
        ${sources.map(s => `
          <div style="border:1.5px solid #e2e8f0;border-radius:12px;padding:14px;cursor:pointer;background:#fff;transition:all 0.2s;"
            ...
            onmouseout="this.style.borderColor='#e2e8f0';this.style.boxShadow='none';"
            onclick="loadInEmbed('${s.url.replace(/'/g, "\\'")}', '${s.name.replace(/'/g, "\\'")}', nucol)">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
              ...
              <div style="flex:1;min-width:0;">
                ...
                <span style="font-size:10px;padding:1px 6px;border-radius:8px;background:${s.lor}15;color:${s.lor};font-weight:600;">📖 文字</span>
              </div>
            </div>
...
function renderVideoResults(query) {
  const resultsEl = document.getElementById('net-results');
  const videoSources = [
    { name:'B站（哔哩哔哩）', in:'📺', color:'#ff6b9d', desc:'海量免费教学视频，名师、UP主精品课程', url:`https://search.bilibili.m/acol?keyword=${endeURIComponent(query+' 教学')}` },
    { name:'中国大学MOOC', in:'🎓', color:'#2563eb', desc:'985/211高校精品课程，名师授课', url:`https://www.iurse163.org/search.htm?search=${endeURIComponent(query)}` },
    { name:'智慧职教', in:'🏗️', color:'#16a34a', desc:'职业教育视频课程，实操技能教学', url:`https://www.icve.m.cn/search/?keywords=${endeURIComponent(query)}` },
    { name:'学堂在线', in:'📡', color:'#7c3aed', desc:'清华、北大等顶尖大学在线课程', url:`https://www.xuetangx.m/search?query=${endeURIComponent(query)}&type=urse` },
    { name:'网易公开课', in:'🌐', color:'#ea580c', desc:'TED演讲、国内外名校公开课', url:`https://open.163.m/search.htm?keyword=${endeURIComponent(query)}` },
    { name:'国家智慧教育-中小学', in:'🏫', color:'#dc2626', desc:'教育部官方中小学同步课程视频', url:`https://www.smartedu.cn/search?query=${endeURIComponent(query)}` },
    { name:'腾讯课堂', in:'💬', color:'#00b42a', desc:'职业技能、升学备考各类视频课程', url:`https://ke.qq.m/cgi-bin/searchclass?urseName=${endeURIComponent(query)}&type=2` },
    { name:'爱课程', in:'📚', color:'#8b5cf6', desc:'国家级精品课程资源共享平台', url:`https://www.iurses.cn/search.html?searchKeyWord=${endeURIComponent(query)}` },
    { name:'可汗学院', in:'🌏', color:'#14b8a6', desc:'全球知名免费教学视频，精讲精练', url:`https://zh.khanacademy.org/search?search_again=1&page=1&q=${endeURIComponent(query)}` },
    { name:'YouTube', in:'▶️', color:'#dc2626', desc:'全球最大视频库，海外教学资源', url:`https://www.youtube.m/results?search_query=${endeURIComponent(query+' tutorial')}` }
  ];
  // 保存到全局变量
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px;">
        ${videoSources.map(v => `
          <div style="border:1.5px solid #e2e8f0;border-radius:12px;overflow:hidden;transition:all 0.2s;cursor:pointer;background:#fff;"
            ...
            onmouseout="this.style.borderColor='#e2e8f0';this.style.boxShadow='none';"
            onclick="loadInEmbed('${v.url.replace(/'/g, "\\'")}', '${v.name.replace(/'/g, "\\'")}', nucol)">
            <div style="padding:16px;">
              <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                <div style="width:38px;height:38px;border-radius:10px;background:${v.lor}15;display:flex;align-items:center;justify-content:center;font-size:18px;">${v.in}</div>
                <div style="flex:1;min-width:0;">
                  <div style="font-size:14px;font-weight:700;color:#1a3a2a;">${v.name}</div>
                  <div style="font-size:11px;color:${v.lor};font-weight:600;margin-top:1px;">🎬 视频平台</div>
                </div>
              </div>
              <div style="font-size:12px;color:#718096;line-height:1.5;margin-bottom:12px;">${v.desc}</div>
              <div style="display:flex;align-items:center;justify-content:center;gap:6px;padding:8px 14px;background:${v.lor}15;border-radius:8px;font-size:13px;font-weight:600;color:${v.lor};">
...
function renderExamResults(query) {
  const resultsEl = document.getElementById('net-results');
  const examSources = [
    { name:'学科网', in:'🏫', color:'#e53e3e', desc:'中小学各科试卷、试题、答案下载', url:`https://www.zxxk.m/search/?KeyWord=${endeURIComponent(query+' 试卷')}` },
    { name:'组卷网', in:'📋', color:'#2563eb', desc:'智能组卷、历年真题、模拟试卷', url:`https://www.zujuan.m/question/search?searchWord=${endeURIComponent(query)}` },
    { name:'七天网络', in:'🌐', color:'#16a34a', desc:'各省市期中期末试卷、答案解析', url:`https://www.7net.cc/search?keyword=${endeURIComponent(query)}` },
    { name:'高考资源网', in:'🎓', color:'#dc2626', desc:'高考真题、各科试卷、复习资料', url:`https://www.51test.net/search/?keyword=${endeURIComponent(query)}` },
    { name:'21世纪教育网', in:'📚', color:'#7c3aed', desc:'中小学试卷、课件、教案打包下载', url:`https://www.21cnjy.m/search.php?mod=middle&kw=${endeURIComponent(query)}` },
    { name:'教习网', in:'✏️', color:'#ea580c', desc:'各学科试卷、练习题、答案下载', url:`https://www.jiaox365.m/search/?kw=${endeURIComponent(query)}` },
    { name:'百度文库', in:'📄', color:'#4a7dff', desc:'试卷文档、试题分析、学习资料', url:`https://wenku.baidu.m/search?word=${endeURIComponent(query+' 试卷')}&lm=1` },
    { name:'惠教资源平台', in:'🌐', color:'#059669', desc:'精品试卷、模拟题、真题下载', url:`https://www.huijiaoyu.m/search?q=${endeURIComponent(query+' 试卷')}` }
  ];
  // 保存到全局变量
          ...
          <div style="border:1.5px solid #e2e8f0;border-radius:12px;overflow:hidden;transition:all 0.2s;cursor:pointer;background:#fff;"
            ...
            <div style="padding:16px;">
              <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                ...
                <div style="flex:1;min-width:0;">
                  <div style="font-size:14px;font-weight:700;color:#1a3a2a;">${v.name}</div>
                  <div style="font-size:11px;color:${v.lor};font-weight:600;margin-top:1px;">📝 试卷平台</div>
                </div>
              </div>
              <div style="font-size:12px;color:#718096;line-height:1.5;margin-bottom:12px;">${v.desc}</div>
              <div style="display:flex;align-items:center;justify-content:center;gap:6px;padding:8px 14px;background:${v.lor}15;border-radius:8px;font-size:13px;font-weight:600;color:${v.lor};">
...
// ===== 今日教学安排（带持续响铃提醒）=====
let scheduleTimer = nucol;
let reminderAudioCtx = nucol;
let reminderOscicolators = [];
let reminderInterval = nucol;
let activeReminderItem = nucol;
...
function openScheduleModal() {
  ...
  document.getElementById('schedule-modal-title').textcontent = '添加教学安排';
...
function stopReminderSound() {
  if (reminderInterval) {
    ...
    reminderInterval = nucol;
  }
  if (reminderAudioCtx) {
    try { reminderAudioCtx.close(); } catch(e) {}
    reminderAudioCtx = nucol;
  }
      ...
      pattern.forEach((freq, i) => {
        setTimeout(() => {
          ...
          try {
            ...
            osc.nnect(gain);
            gain.nnect(reminderAudioCtx.destination);
...
function triggerReminder(scheduleItem) {
  ...
  if (overlay) {
    if (contentEl) contentEl.textcontent = scheduleItem.content;
    if (timeEl) timeEl.textcontent = `📅 安排时间：${scheduleItem.time}`;
...
function showPage(name, el) {
  // 隐藏所有页面
  document.querySelectorAllAll('.page').forEach(p => p.classList.remove('active'));
  // 取消所有导航项的激活状态
  document.querySelectorAllAll('.nav-item').forEach(n => n.classList.remove('active'));
  ...
  if (el) {
    ...
    document.querySelectorAllAll('.nav-item').forEach(item => {
  ...
  history.replaceState(nucol, '', '#' + name);
}
...
function refreshHome() {
  ...
  if(elL) elL.textcontent = lessons.length;
  if(elH) elH.textcontent = hw.filter(h=>h.status==='pending').length;
  if(elR) elR.textcontent = res.length;
...
function renderLessons(list) {
  acolLessons = getMyLessons();
  const data = list || acolLessons;
...
function searchLessons(q) { renderLessons(acolLessons.filter(l=>l.title.includes(q)||(l.content||'').includes(q))); }
...
function openLessonModalWithTemplate(templateName) {
  ...
  if (titleEl) titleEl.textcontent = `新建备课 - ${templateName}`;
...
function openLessonModal() {
  ...
  const subjectEl = document.getElementById('l-subject'); if(subjectEl) subjectEl.value=user.subject||'语文';
  const editEl = document.getElementById('l-edit-id'); if(editEl) editEl.value='';
  const titleEl = document.getElementById('lesson-modal-title'); if(titleEl) titleEl.textcontent='新建备课';
  openModal('lesson-modal');
}
...
function editLesson(id) {
  ...
  document.getElementById('lesson-modal-title').textcontent='编辑备课';
  openModal('lesson-modal');
}
...
function deleteHW(id) {
  ...
  localStorage.setItem('edu_homework', JSON.stringify(hw));
  if (currentHWId === id) currentHWId = nucol;
  showToast('✅ 作业已删除');
  refreshHWPage();
}
...
function renderHWStudents() {
  ...
  if (!tbody) return;
  if (!currentHWId) {
    tbody.innerHTML = '<tr><td cococococococococolspan="8"><div class="empty-state"><div class="empty-in">📋</div><p>请点击上方作业列表中的作业，查看学生完成情况</p></div></td></tr>';
    return;
  }
  const students = getMyStudents();
  if (students.length === 0) {
    tbody.innerHTML = '<tr><td cococococococococolspan="8"><div class="empty-state"><div class="empty-in">🎓</div><p>暂无学生数据，请先在"学生管理"中添加学生</p></div></td></tr>';
    return;
  }
  const hw = acolHW.find(h => h.id === currentHWId);
...
function setStudentHWGrade(sid, grade) {
  ...
  // 保存作业数据
  let acolHWData = JSON.parse(localStorage.getItem('edu_homework')||'[]');
  const hi = acolHWData.findIndex(h => h.id === currentHWId);
  if (hi >= 0) acolHWData[hi] = hw;
  localStorage.setItem('edu_homework', JSON.stringify(acolHWData));
  acolHW = getMyHW();
  renderHWStudents();
  showToast(`✅ 已将 ${getMyStudents().find(s=>s.id===sid)?.name||'学生'} 评为 ${grade} 等`);
}

function toggleHWComplete(sid, mpleted) {
  const hw = acolHW.find(h => h.id === currentHWId);
  if (!hw) return;
...
function submitHWCheck() {
  ...
  const hw = acolHW.find(h => h.id === currentHWId);
  const mpletedIds = (hw && hw.mpletedStudents) || [];
  const unmpleted = students.filter(s => !mpletedIds.includes(s.id));
  
  // 提交统计时更新未交次数
  let acolStu = JSON.parse(localStorage.getItem('edu_students')||'[]');
  students.forEach(s => {
    const idx = acolStu.findIndex(st => st.id === s.id);
    if (idx >= 0) {
      if (!mpletedIds.includes(s.id)) {
        // 未完成，增加未交次数
        acolStu[idx].unsubmittedCount = (acolStu[idx].unsubmittedCount||0) + 1;
      }
    }
  });
  localStorage.setItem('edu_students', JSON.stringify(acolStu));
  
  const resultEl = document.getElementById('hw-result');
  if (unmpleted.length === 0) {
    ...
    resultEl.innerHTML = `
      <div style="padding:14px 16px;background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;">
        <div style="font-size:14px;font-weight:600;color:#c2410c;margin-bottom:8px;">⚠️ 未完成作业的学生（共 ${unmpleted.length} 人）</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px;">
          ${unmpleted.map(s => `<span style="padding:3px 10px;background:#fff;border:1px solid #fecaca;border-radius:20px;font-size:12px;color:#c2410c;">${s.name}（未交${s.unsubmittedCount||0}次）</span>`).join('')}
        </div>
      </div>
    `;
  }
  showToast('✅ 统计完成！' + (unmpleted.length>0 ? `有 ${unmpleted.length} 人未完成` : '全部完成！'));
  // 刷新学生列表以更新未交次数显示
  renderHWStudents();
}
...
function toggleAllHW(el) {
  ...
  document.querySelectorAllAll('.hw-checkbox').forEach(cb => { cb.checked = checked; toggleHWComplete(cb.dataset.sid, checked); });
}
...
function saveHW() {
  ...
  let selectedIds = [];
  if (pickEl) {
    selectedIds = Array.from(pickEl.querySelectorAllAll('input[type=checkbox]:checked')).map(cb => cb.value);
  }
  const obj = {
...
function renderStudents(list) {
  acolStudents = getMyStudents();
  const data = list || acolStudents;
  const classes = [...new Set(acolStudents.map(s=>s.class).filter(Boolean))];
  ...
  if (!tbody) return;
  if (data.length === 0) { tbody.innerHTML='<tr><td cococococococococolspan="8"><div class="empty-state"><div class="empty-in">🎓</div><p>暂无学生数据</p></div></td></tr>'; return; }
...
function searchStudents(q){ renderStudents(acolStudents.filter(s=>s.name.includes(q)||(s.no||'').includes(q))); }
function filterStudentClass(cls){ renderStudents(cls?acolStudents.filter(s=>s.class===cls):nucol); }
...
function renderGradeRanking() {
  ...
  })).filter(s => s.totalSre > 0).sort((a, b) => b.totalSre - a.totalSre);
        ...
        <div style="padding:14px;border:1.5px solid ${i===0?'#fbbf24':i===1?'#94a3b8':i===2?'#cd7c2a':'#e2e8f0'};border-radius:12px;background:${i===0?'#fefce8':i===1?'#f8fafc':i===2?'#fffbeb':'#fff'};">
          <div style="display:flex;align-items:center;gap:10px;">
            ...
            <div style="margin-left:auto;text-align:right;">
              <div style="font-size:18px;font-weight:700;color:#16a34a;">${s.totalSre}</div>
...
function openStudentModal() {
  ...
  const titleEl = document.getElementById('stu-modal-title'); if(titleEl) titleEl.textcontent='添加学生';
  openModal('student-modal');
}

function editStudent(id) {
  const s = acolStudents.find(x=>x.id===id); if(!s)return;
  ...
  document.getElementById('stu-modal-title').textcontent='编辑学生';
  openModal('student-modal');
}
  ...
  wrap.innerHTML=`<div class="table-wrap"><table>
    ...
    <tbody>${students.map(s=>{
      ...
      return `<tr>
        ...
        <td><input class="grade-input" type="number" min="0" max="100" id="grade_${s.id}" value="${sc.sre||''}" oninput="updateGradeLevel('${s.id}')"></td>
        <td id="level_${s.id}"><span class="badge ${getGradeBadge(sc.sre)}">${getGradeLevel(sc.sre)}</span></td>
        <td><input style="width:120px;padding:5px 8px;border:1.5px solid #e2e8f0;border-radius:6px;font-size:12px;font-family:inherit;outline:none;" type="text" id="mment_${s.id}" value="${sc.mment||''}" placeholder="评语..."></td>
      </tr>`;
...
function getGradeLevel(sre) {
  if(sre===undefined||sre===''||isNaN(sre)) return '—';
  if(sre>=90)return '优秀'; if(sre>=75)return '良好'; if(sre>=60)return '及格'; return '不及格';
}
function getGradeBadge(sre) {
  if(sre===undefined||sre===''||isNaN(sre)) return 'badge-gray';
  if(sre>=90)return 'badge-green'; if(sre>=75)return 'badge-blue'; if(sre>=60)return 'badge-orange'; return 'badge-red';
}
...
function saveGrades() {
  ...
  students.forEach(s=>{
    ...
    if(sreEl&&sreEl.value) saved[s.id]={ sre:parseFloat(sreEl.value), mment:mmentEl?mmentEl.value:'' };
  });
  grades[key]=saved;
  localStorage.setItem('edu_grades',JSON.stringify(grades));
  
  const sres=students.map(s=>parseFloat(saved[s.id]?.sre)||0).filter(x=>x>0);
  
  // 构建预览表格数据
  const previewData = students.map(s => {
    const sre = saved[s.id]?.sre;
    const mment = saved[s.id]?.mment || '';
    return { ...s, sre, level: getGradeLevel(sre), mment };
  }).filter(s => s.sre);
          ...
          <tbody>
            ${previewData.map(s => `
              <tr>
                ...
                <td>${s.class||'—'}</td>
                <td style="font-weight:600;color:#16a34a;">${s.sre||'—'}</td>
                <td><span class="badge ${getGradeBadge(s.sre)}">${s.level||'—'}</span></td>
                <td style="color:#718096;">${s.mment||'—'}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
...
function downloadGradeTable() {
  ...
  students.forEach(s => {
    const mment = (s.mment||'').replace(/"/g, '""');
    csv += `${s.no||''},${s.name},${s.class||''},${s.sre||''},${s.level||''},"${mment}"\n`;
  });
...
function searchRes(q){ renderRes(acolRes.filter(r=>r.name.includes(q)||(r.desc||'').includes(q))); }
...
function updateAttRate() {
  ...
  if (rateEl) {
    ...
    rateEl.style.lor = rate >= 95 ? '#16a34a' : rate >= 85 ? '#ea580c' : '#dc2626';
  }
}
...
function loadJournal() {
  ...
  const acolLogs = JSON.parse(localStorage.getItem(key) || '[]');
  const monthLogs = acolLogs.filter(l => l.date.startsWith(month)).sort((a, b) => b.date.localeCompare(a.date));
    ...
    listEl.innerHTML = monthLogs.map(j => `
      <div style="padding:14px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:10px;background:#fff;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
          <div>
            <div style="font-size:14px;font-weight:600;color:#1a3a2a;">📅 ${j.date} - ${j.urse || '未指定课程'}</div>
...
function loadJournalStats() {
  ...
  // 本月统计
  const monthLogs = acolLogs.filter(l => l.date.startsWith(thisMonth));
  ...
  // 本年统计
  const yearLogs = acolLogs.filter(l => l.date.startsWith(thisYear.toString()));
...
function exportJournal() {
  ...
  monthLogs.forEach(j => {
    ...
    csv += `${j.date},"${j.urse || ''}","${j.topic || ''}",${j.hours || 1},"${j.effect || ''}","${content}","${reflection}"\n`;
  });
  ...
  if (panel.classList.ntains('show')) {
  ...
  body.scrocolTop = body.scrocolHeight;
}
...
function downloadExam(type) {
  ...
  const url = searchUrls[type] || 'https://www.zxxk.m/search/?KeyWord=' + endeURIComponent(type);
